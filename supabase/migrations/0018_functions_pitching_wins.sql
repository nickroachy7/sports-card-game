-- ─────────────────────────────────────────────────────────────────────────
-- 0018_functions_pitching_wins.sql — close the pitching_wins milestone.
--
-- Extends _finalize_contest_entry's milestone counter aggregation to
-- include the 'mlb.game.pitcher_win' event type, which the post-game
-- reconciliation pass (lib/mlb/reconcile.ts) now emits for each
-- winning pitcher it identifies. Counts matches against starter card
-- player_id via game_event.pitcher_player_id.
--
-- Only the pitching_wins branch of _finalize_contest_entry changes;
-- everything else is identical to 0017's version.
-- ─────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Batter-based counters (hits / HR / SB).
  SELECT
  ...
  -- Pitcher-based counter (wins). Counts synthetic 'mlb.game.pitcher_win'
  -- events emitted by the post-game reconciliation pass.
  SELECT COUNT(*)
  INTO v_wins
  FROM public.contest_lineup_slot s
  JOIN public.card c        ON c.id = s.starter_card_id
